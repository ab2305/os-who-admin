  ...
  <div>
    <el-form :model="form" :rules="rules" label-width="120px"
      ...
      <el-form-item>
        ...
        <router-link to="/operation/privacies" tag="el-button">취소</router-link>
      </el-form-item>
    </el-form>
  </div>
</template>
		...
		write() {
			this.$refs.form.validate(async (isValid) => {
				...
				try {
					...
					this.$router.push('/operation/privacies')

					this.$notify.success({
						title: this.isEdit ? '수정' : '등록',
						message: this.isEdit ? '개인정보취급방침을 수정했습니다.' : '개인정보취급방침을 등록했습니다.',
					})
				} catch (error) {
					this.$notify.error({
